...
class Analysis:
    ...
    def plot_true_hit_scores_scatter_by_iters(
            self,
            experiment_hits_path: str = f"{PROJ_DIR}/results/rdkit_desc/plots/hit_discovery.json",
            docking_results_path: str = f"{PROJ_DIR}/datasets/hits/hits_targets.csv",
            docking_column: str = "Affinity(kcal/mol)",
            allowed_experiments: list = None,
            iters_to_plot: list = [0, 5, 10, 15, 20, 25, 30],
            out_prefix: str = "hits_scatter_iter",
        ...
        """
        Scatter plots showing **true docking-score hits** at specific iterations.
        One figure per iteration in `iters_to_plot`.
        """
        import numpy as np
        import pandas as pd
        import matplotlib.pyplot as plt
        ...
        # Load experiment hits
        with open(experiment_hits_path, "r") as f:
            experiment_hits = json.load(f)

        if allowed_experiments:
            experiments = [e for e in experiment_hits if e in allowed_experiments]
        else:
            experiments = list(experiment_hits.keys())

        # Load docking scores from curated hits file
        ...
        for fpath in docking_files:
            ...
            docking_map.update(pd.to_numeric(df[docking_column], errors="coerce").dropna().to_dict())

        rng = np.random.default_rng(seed)

        # Loop over requested iterations
        for it in iters_to_plot:
            rows = []
            for exp in experiments:
                it_list = experiment_hits[exp]["it_hit_ids"]
                if it >= len(it_list):
                    continue  # skip if iteration not available
                hit_ids = it_list[it]
                for mid in hit_ids:
                    if mid in docking_map:
                        rows.append({"experiment": exp, "ID": mid, "docking_score": docking_map[mid]})
            if not rows:
                print(f"No hits found for iteration {it}")
                continue

            plot_df = pd.DataFrame(rows)

            # Prepare plotting
            exp_order = sorted(experiments)
            x_positions = {exp: i * 0.6 for i, exp in enumerate(exp_order)}
            exp_labels = {exp: exp.split("_")[-1].upper() for exp in exp_order}

            plot_df["x"] = [x_positions[e] + rng.uniform(-jitter, jitter) for e in plot_df["experiment"]]
            ...
            for exp, xpos in x_positions.items():
                ...
                if pd.notnull(med):
                    plt.hlines(med, xpos - 0.35, xpos + 0.35,
                            colors="black", linestyles="dashed", linewidth=2, alpha=0.8)

            plt.tight_layout()
            plt.legend(title="Experiments", bbox_to_anchor=(1.02, 1),
                    loc="upper left", frameon=False,
                    fontsize=legend_fontsize, title_fontsize=legend_fontsize)

            out_path = f"{PROJ_DIR}/results/rdkit_desc/plots/{out_prefix}_iter{it}.png"
            plt.savefig(out_path, dpi=300, bbox_inches="tight")
            plt.show()
            print(f"Saved scatter for iteration {it}: {out_path}")
