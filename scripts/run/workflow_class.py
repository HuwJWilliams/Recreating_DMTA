...
class RecDMTA:
    ...
    def SelectCmpds(self, iteration: int, prev_it_dir: str, frac: float = 0.1):
        ...
        if self.selection_method == "r":
            ...
            sel_idx = self.sel.random_in_best(
                column="pred_Affinity(kcal/mol)", ascending=False, frac=self.sel_size
            )
    ...
    def RunDocking(
        ...
        for args in sjw_args:
            docker, job_ids, ds_batch_file, mols_to_edit_ls, idx_ls = (
                self._submit_jobs_wrapper(args)
            )
        ...
        if missing_ids:
            print(f"Missing IDs found:\n{missing_ids}")
            for mid in missing_ids:
                try:
                    batch_no = molid2batchno(molid=mid, prefix=self.id_prefix, dataset_file=self.docking_score_files)
                    dock_batch_csv = self.docking_score_files.replace("*", str(batch_no))
                    ...
                    if mid in dock_batch_df.index:
                        ...
                        print(f"Updated missing ID: {mid}")
                    else:
                        print(f"ID {mid} not found in docking CSV: {dock_batch_csv}")

                except Exception as e:
                    print(f"Error updating missing ID {mid}:\n{e}")
    ...
    def _predict_for_files(self, args: list):
        ...
        logger.info(f"Predicting on descriptor file: {desc_file} and full file: {full_file}")
        try:
            ...
            logger.info(f"Prediction for {desc_file} completed successfully.")
        except Exception as e:
            logger.error(f"Error during prediction for {desc_file}: {e}")
    ...
    def RunIterations(self, held_out_test_feats: str, held_out_test_targs: str):
        ...
        for self.iter in range(self.start_iter, self.start_iter + self.total_iters):
            ...
            if self.time_ran + (self.avg_runtime * 1.5) < self.max_it_runtime:
                ...
                self.SelectCmpds(iteration=self.iter, prev_it_dir=self.prev_it_dir, frac=self.sel_size)
                ...
                held_out_targ_df = held_out_targ_df[
                    held_out_targ_df[self.docking_column] != "False"
                ]
                ...
                bias, sdep, mse, rmse, r2, pearson_r, pearson_p, true, pred = (
                    model._calculate_performance(
